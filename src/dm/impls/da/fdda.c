  ...
  */
  ...
  if (!aij) {
    ...
    if (!baij) {
      ...
      ierr = PetscObjectQueryFunction((PetscObject)A,"MatMPISGSetPreallocation_C",&sg);CHKERRQ(ierr);
      if (!sg) {
        ierr = PetscObjectQueryFunction((PetscObject)A,"MatSeqSGSetPreallocation_C",&sg);CHKERRQ(ierr);
      }
    }
