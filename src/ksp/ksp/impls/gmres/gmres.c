...
PetscErrorCode KSPView_GMRES(KSP ksp,PetscViewer viewer)
{
  ...
  if (iascii) {
    ierr = PetscViewerASCIIPrintf(viewer,"  GMRES: %H=%D, using %H\n","KSPGMRESSetRestart","restart",gmres->max_k,"KSPGMRESSetOrthogonalization",cstr);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"  GMRES: %H %G\n","KSPGMRESSetHapTol","happy breakdown tolerance",gmres->haptol);CHKERRQ(ierr);
  } else if (isstring) {
