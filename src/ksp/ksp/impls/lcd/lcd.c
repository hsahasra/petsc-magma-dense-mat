...
PetscErrorCode KSPView_LCD(KSP ksp,PetscViewer viewer)
{
  ...
  if (iascii) {
    ierr = PetscViewerASCIIPrintf(viewer,"  LCD: %H=%d\n","KSPLCDSetRestart","restart",lcd->restart);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"  LCD: %H %g\n","KSPLCDSetHapTol","happy breakdown tolerance",lcd->haptol);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
