...
if(!b_a){
    ierr = PetscMalloc(bs2*sizeof(PetscInt), &(b_a)); CHKERRQ(ierr);


	memset(b_a, 0,sizeof(PetscInt)*bs2);
#ifdef _VEC4
	strtval = 0;
	for(j=0; (j+4) <= bs; j+=4){
		for(i = 0; (i+4) <= bs ; i+=4,strtval += 16){
			for(k = 0; k < 2 ; k++ ){
				b_a [i*bs+k+j] = strtval+k;
				b_a [i*bs+k+j+2] = strtval+4+k;
				b_a [(i+1)*bs+k+j] = strtval+8+k;
				b_a [(i+1)*bs+k+j+2] = strtval+12+k;
			}
			for(k = 2; k < 4 ; k++ ){
				b_a [(i+2)*bs+k+j] = strtval+k;
				b_a [(i+2)*bs+k+j-2] = strtval+4+k;
				b_a [(i+3)*bs+k+j] = strtval+8+k;
				b_a [(i+3)*bs+k+j-2] = strtval+12+k;
			}
		}
		for(; (i+2) <= bs ; i+=2,strtval += 8){
			b_a [i*bs+j] = strtval+0;
			b_a [i*bs+j+1] = strtval+1;
			b_a [i*bs+j+2] = strtval+2;
			b_a [i*bs+j+3] = strtval+3;
			b_a [(i+1)*bs+j] = strtval+4;
			b_a [(i+1)*bs+j+1] = strtval+5;
			b_a [(i+1)*bs+j+2] = strtval+6;
			b_a [(i+1)*bs+j+3] = strtval+7;
		}
	...
	for(; (j+2) <= bs; j+=2){
		for(i = 0; (i+4) <= bs ; i+=4,strtval += 8){
			b_a [i*bs+j] = strtval+0;
			b_a [i*bs+j+1] = strtval+1;
			b_a [(i+2)*bs+j] = strtval+2;
			b_a [(i+2)*bs+j+1] = strtval+3;
			b_a [(i+1)*bs+j] = strtval+4;
			b_a [(i+1)*bs+j+1] = strtval+5;
			b_a [(i+3)*bs+j] = strtval+6;
			b_a [(i+3)*bs+j+1] = strtval+7;
		}
		for(; (i+2) <= bs ; i+=2,strtval += 4){
			b_a [i*bs+j] = strtval+0;
			b_a [i*bs+j+1] = strtval+1;
			b_a [(i+1)*bs+j] = strtval+2;
			b_a [(i+1)*bs+j+1] = strtval+3;
		}
	...
	for(; j < bs; j++){
		for(i = 0; (i+4) <= bs ; i+=4,strtval += 4){
			b_a [i*bs+j] = strtval+0;
			b_a [(i+1)*bs+j] = strtval+1;
			b_a [(i+2)*bs+j] = strtval+2;
			b_a [(i+3)*bs+j] = strtval+3;
		}
  ...
  for (i=0; i<n; i++) {
    ...
    for(m=0;m<nz;m++){
      ...
      for(k=0;k<15;k++){
	xv        = x[k + idx];
	x[idt]    -= v[b_a[k+15*0]]*xv;
	x[1+idt]  -= v[b_a[k+15*1]]*xv;
	x[2+idt]  -= v[b_a[k+15*2]]*xv;
        x[3+idt]  -= v[b_a[k+15*3]]*xv;
	x[4+idt]  -= v[b_a[k+15*4]]*xv;
	x[5+idt]  -= v[b_a[k+15*5]]*xv;
	x[6+idt]  -= v[b_a[k+15*6]]*xv;
        x[7+idt]  -= v[b_a[k+15*7]]*xv;
	x[8+idt]  -= v[b_a[k+15*8]]*xv;
	x[9+idt]  -= v[b_a[k+15*9]]*xv;
	x[10+idt] -= v[b_a[k+15*10]]*xv;
        x[11+idt] -= v[b_a[k+15*11]]*xv;
	x[12+idt] -= v[b_a[k+15*12]]*xv;
	x[13+idt] -= v[b_a[k+15*13]]*xv;
	x[14+idt] -= v[b_a[k+15*14]]*xv;
      }
	v += 225;
    }
  }
  ...
  for (i=n-1; i>=0; i--){
	...
	s[8]  -= v[b_a[k+15*8]]*xv;
	s[9]  -= v[b_a[k+15*9]]*xv;
	s[10] -= v[b_a[k+15*10]]*xv;
        s[11] -= v[b_a[k+15*11]]*xv;
	s[12] -= v[b_a[k+15*12]]*xv;
	s[13] -= v[b_a[k+15*13]]*xv;
	s[14] -= v[b_a[k+15*14]]*xv;
      }
	v += 225;
    }
    ierr = PetscMemzero(x+idt,bs*sizeof(MatScalar));CHKERRQ(ierr);
    for(k=0;k<15;k++){
      x[idt]    += v[b_a[k+15*0]]*s[k];
      x[1+idt]  += v[b_a[k+15*1]]*s[k];
      x[2+idt]  += v[b_a[k+15*2]]*s[k];
      x[3+idt]  += v[b_a[k+15*3]]*s[k];
      x[4+idt]  += v[b_a[k+15*4]]*s[k];
      x[5+idt]  += v[b_a[k+15*5]]*s[k];
      x[6+idt]  += v[b_a[k+15*6]]*s[k];
      x[7+idt]  += v[b_a[k+15*7]]*s[k];
      x[8+idt]  += v[b_a[k+15*8]]*s[k];
      x[9+idt]  += v[b_a[k+15*9]]*s[k];
      x[10+idt] += v[b_a[k+15*10]]*s[k];
      x[11+idt] += v[b_a[k+15*11]]*s[k];
      x[12+idt] += v[b_a[k+15*12]]*s[k];
      x[13+idt] += v[b_a[k+15*13]]*s[k];
      x[14+idt] += v[b_a[k+15*14]]*s[k];
    }
      v += 225;
  }
  //  for(k=0;k<15;k++){
