...
#include "../src/mat/impls/blockstructgrid/mpi/matbsgmpi.h"
...
/*0*/ MatSetValues_MPIBSG,0,0,MatMult_MPIBSG,0,
/*5*/0,0,0,0,0,
/*10*/0,0,0,0,0,
/*15*/0,0,0,0,0,
/*20*/0,0,0,MatZeroEntries_MPIBSG,0,
/*25*/0,0,0,0,MatSetUpPreallocation_MPIBSG,
/*30*/0,0,0,0,0,
/*35*/0,0,0,0,0,
/*40*/0,0,0,0,0,
/*45*/0,0,0,0,0,
/*50*/0,0,0,0,0,
/*55*/0,0,0,MatSetValuesBlocked_MPIBSG,MatGetSubMatrix_MPIBSG,
/*60*/MatDestroy_MPIBSG,MatView_MPIBSG,0,0,0,
/*65*/0,0,MatSetValues_MPIBSG,0,0,
/*70*/0,0,0,0,0,
/*75*/0,0,0,0,0,
/*80*/0,0,0,0,0,
/*85*/0,0,MatSetValuesBlocked_MPIBSG,0,0,
/*90*/0,0,0,0,0,
/*95*/0,0,0,0,0,
/*100*/0,0,0,0,0,
/*105*/0,0,0,0,0,
/*110*/0,0,0,0,0,
/*115*/MatCreate_MPIBSG,0,0,0,0,
/*120*/0,0,0,0,0,
/*125*/0,0,0,0,0,
/*130*/MatSetStencil_MPIBSG,MatSetGrid_MPIBSG
};


/** MatSetGrid_MPIBSG : Sets the 3d physical grid information
Added by Deepan */
#undef __FUNCT__ 
#define __FUNCT__ "MatSetGrid_MPIBSG"

PetscErrorCode MatSetGrid_MPIBSG(Mat B, PetscInt m, PetscInt n, PetscInt p)
{
	Mat_MPIBSG * b = (Mat_MPIBSG*) B->data;
	PetscFunctionBegin;
...
/** MatCreate_MPIBSG : Creates the struct grid representation 
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatCreate_MPIBSG"

PetscErrorCode MatCreate_MPIBSG(Mat B)
{
	Mat_MPIBSG * b;
	PetscErrorCode ierr;
	...
	ierr = PetscMalloc(sizeof(Mat_MPIBSG),&b);CHKERRQ(ierr);CHKERRQ(ierr);
	B->data = (void *)b;
	...
	b->lbeg = PETSC_NULL;
	b->lend = PETSC_NULL;
	b->rankmap = PETSC_NULL;

	b->bs = 0;
	ierr = PetscObjectChangeTypeName((PetscObject)B, MATMPIBSG); CHKERRQ(ierr);
	PetscFunctionReturn(0);	
}

/** MatDestroy_MPIBSG : Destroys the struct grid representation
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatDestroy_MPIBSG"
PetscErrorCode MatDestroy_MPIBSG(Mat A)
{
	Mat_MPIBSG * a = (Mat_MPIBSG *)A->data;
	PetscErrorCode ierr;
	...
	ierr = PetscFree(a->rankmap);CHKERRQ(ierr);
	if(a->sub_matrix)
	{
...
/** MatMult_MPIBSG : Performs the Matrix - Vector multiplication y= mat*x on the struct grid representation
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatMult_MPIBSG"
PetscErrorCode MatMult_MPIBSG(Mat mat, Vec x, Vec y)
{
/*	PetscErrorCode ierr;
	Mat_MPIBSG * a = (Mat_MPIBSG *) mat->data;
...
/** MatSetValuesBlocked_MPIBSG : Sets the values in the matrix
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatSetValuesBlocked_MPIBSG"

PetscErrorCode MatSetValuesBlocked_MPIBSG(Mat A, PetscInt nrow,const PetscInt irow[], PetscInt ncol,const PetscInt icol[], const PetscScalar y[], InsertMode is)
{
	PetscErrorCode ierr;
	PetscFunctionBegin;	

	ierr = 	MatGetSetValues_MPIBSG(A,  nrow, irow, ncol, icol, &y[0], is,PETSC_TRUE); CHKERRQ(ierr);
	PetscFunctionReturn(0);
}	
...
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatSetValues_MPIBSG"

PetscErrorCode MatSetValues_MPIBSG(Mat A, PetscInt nrow,const PetscInt irow[], PetscInt ncol ,const PetscInt icol[],const PetscScalar y[], InsertMode is)
{
	PetscErrorCode ierr;
	PetscFunctionBegin;	
...
PetscErrorCode MatGetSetValues_MPIBSG(Mat A, PetscInt nrow,const PetscInt irow[], PetscInt ncol ,const PetscInt icol[],PetscScalar *y, InsertMode is, PetscBool SetVal)
{
	PetscErrorCode ierr;
	Mat_MPIBSG * mat = (Mat_MPIBSG *) A->data;
	...
	llimit = mat->stencil_local_rbeg;
	ulimit = mat->stencil_local_rend;
	
	for(i=0;i< nrow ; i++)
	{
		rshift = irow[i]*nstr + rbeg;
		if(rshift < llimit || rshift >= ulimit)
			continue;
		for(j=0;j<ncol;j++)
		{
	...
	if(SetVal)
	{
			ierr = SetValues_Matrix_MPIBSG(mat,count,idx,idy,idz,ioffsets,y,is, rank, size); CHKERRQ(ierr);
	}
	else
	{
			ierr = GetValues_Matrix_MPIBSG(mat,count,idx,idy,idz,ioffsets,y, rank, size); CHKERRQ(ierr);
	}
...
PetscErrorCode GetValues_Matrix_MPIBSG(Mat_MPIBSG *  mat, PetscInt n , const PetscInt idx[], const PetscInt idy[],const PetscInt idz[], const PetscInt ioffsets[], PetscScalar *data, PetscInt rank, PetscInt size)
{
	PetscErrorCode ierr;
...
/** MatSetValuesBlocked_MPIBSG : Sets the values in the matrix with the 3d indices supplied
Added by Deepan */
PetscErrorCode SetValues_Matrix_MPIBSG(Mat_MPIBSG *  mat, PetscInt n , const PetscInt idx[], const PetscInt idy[],const PetscInt idz[], const PetscInt ioffsets[], const  PetscScalar data[], InsertMode is, PetscInt rank, PetscInt size)
{
	PetscErrorCode ierr;
	PetscInt i,j,k,c,endpoint,k1,mx = mat->m, ny= mat->n, dof = mat->dof, bs = mat->bs;
	PetscInt lda1 = mx*ny, lda2 = mx;
...
/** MatSetValuesBlocked_MPIBSG : Sets the values in the matrix with the 3d indices supplied
Added by Deepan *//*
PetscErrorCode SetValues_Matrix_MPIBSG(Mat_MPIBSG *  mat, PetscInt n , const PetscInt idx[], const PetscInt idy[],const PetscInt idz[], const PetscInt ioffsets[], const  PetscScalar data[], InsertMode is)
{
...
/** MatSetStencil_MPIBSG : Sets the stencil neighbor points
 * sets various stencil related info such as neighbor points displacements and dimension of the grid
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatSetStencil_MPIBSG"

PetscErrorCode MatSetStencil_MPIBSG(Mat A, PetscInt dim,const PetscInt dims[],const PetscInt starts[], PetscInt dof)
{
	Mat_MPIBSG * mat = (Mat_MPIBSG *)A->data;
	PetscInt mnos = dim;
	PetscErrorCode ierr;
...
/** MatSetUpPreallocation_MPIBSG : Allocates space for coefficient matrix
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatSetUpPreallocation_MPIBSG"
PetscErrorCode MatSetUpPreallocation_MPIBSG(Mat mat)
{
	Mat_MPIBSG * a = (Mat_MPIBSG *)mat->data;
	PetscErrorCode ierr;
	PetscFunctionBegin;
	if(a->sub_matrix)
	{
		ierr = MatSetUpPreallocation_SubMatrix_MPIBSG(mat); CHKERRQ(ierr);
	}
	else
	{
		ierr = MatSetUpPreallocation_Matrix_MPIBSG(mat); CHKERRQ(ierr);
	}
...
PetscErrorCode MatSetUpPreallocation_SubMatrix_MPIBSG(Mat mat)
{
/*	PetscErrorCode ierr;
	Mat_MPIBSG * a = (Mat_MPIBSG *)mat->data;
	...
	ierr = MatSetUpRegion_MPIBSG(mat); CHKERRQ(ierr);
...
PetscErrorCode MatSetUpSubRegion_MPIBSG(Mat A)
{
	PetscErrorCode ierr;
	Mat_MPIBSG * a = (Mat_MPIBSG*) A->data;
	PetscInt rank, size;
...
PetscErrorCode MatSetUpRegion_MPIBSG(Mat A)
{
	PetscErrorCode ierr;
	PetscInt rank, size;
	Mat_MPIBSG * a = (Mat_MPIBSG*) A->data;
	PetscInt irowbeg = a->stencil_local_rbeg;
	PetscInt irowend = a->stencil_local_rend;
	PetscInt irrowbeg = a->stencil_rbeg;
	PetscInt irrowend = a->stencil_rend;
	...
	for(c=0; c<a->nregion; c++){
		...
		a->lbeg[c] = lb;
		a->lend[c] = le;
		a->rstart[c] = (lstart[c]-irrowbeg );
		if(irrowbeg != 0) a->rstart[c] += 1;
		a->rstart[c] /= nstride;
	}
	a->rstart[a->nregion] = (lstart[a->nregion] - irrowbeg );
		if(irrowbeg != 0) a->rstart[a->nregion] += 1;
		a->rstart[a->nregion] /= nstride;
	
	ierr = MatSetUpSubRegion_MPIBSG(A); CHKERRQ(ierr);
	PetscFree(ioffsets);	
	PetscFree(lstart);
...
PetscErrorCode MatSetUpPreallocation_Matrix_MPIBSG(Mat mat)
{
	PetscErrorCode ierr;
	PetscInt i,c,j, l3threshold, endpoint;
	PetscInt start[8], count = 0;
	PetscInt rank, size;
	PetscInt localnz, nz1, localnz1;
	Mat_MPIBSG * a = (Mat_MPIBSG *)mat->data;
	ierr = MPI_Comm_rank(((PetscObject)mat)->comm,&rank);CHKERRQ(ierr);
	ierr = MPI_Comm_size(((PetscObject)mat)->comm,&size);CHKERRQ(ierr);

//	printf("\n %d %d %d", rank, mat->rmap->n, mat->cmap->n);
	...
	ierr = PetscMalloc(sizeof(PetscInt)*size,&(a->rankmap));CHKERRQ(ierr);
	
	a->stencil_local_rbeg = 0;
	...
	nz1-= localnz;
	for(i=1;i<=rank ; i++){
		a->rankmap[i] = a->rankmap[i-1] + localnz;
		localnz = nz1/ (size-i);
		nz1-= localnz;
	}
	...
	for( ; i< size; i++)
	{
		a->rankmap[i] = a->rankmap[i-1] + localnz1;
		localnz1 = nz1/ (size-i);
		nz1-= localnz1;
	}

	a->stencil_local_rbeg = a->rankmap[rank];
	a->stencil_local_rend = a->stencil_local_rbeg + localnz;

	ierr = PetscMalloc(sizeof(PetscScalar)*localnz*a->bs*a->stpoints,&(a->a));CHKERRQ(ierr);
	//ierr = PetscMalloc(sizeof(PetscInt)*(8),&(a->stpoffset));CHKERRQ(ierr);
	memset(a->a, 0,sizeof(PetscScalar)*localnz*a->bs*a->stpoints);
	ierr = PetscMalloc(sizeof(PetscScalar)*localnz*a->bs,&(a->diag));CHKERRQ(ierr);
	memset(a->diag, 0,sizeof(PetscScalar)*localnz*a->bs);
...
PetscErrorCode MatGetSubMatrix_MPIBSG_Private(Mat A,IS isrow,IS iscol,MatReuse scall,Mat *B)
{
/*	Mat_MPIBSG    *a = (Mat_MPIBSG*)A->data,*c;
	PetscErrorCode ierr;
...
/*	if (scall == MAT_REUSE_MATRIX) {
	...
	ierr = MatSetUpRegion_MPIBSG(*B); CHKERRQ(ierr);
	C = *B;
	} else { 
	 PetscInt dim = 3;
	...
	ierr = MatSetUpPreallocation_MPIBSG(C); CHKERRQ(ierr);
...
//				GetOriginalMatrixBlock
				ierr = MatGetSetValues_MPIBSG(A, 1, &row, 1 , &kstart, val, INSERT_VALUES, PETSC_FALSE); CHKERRQ(ierr);
				xrow = (row-c->stencil_rbeg)/c->stencil_stride;
                                kstart = (kstart-c->stencil_cbeg)/c->stencil_stride;
				MatSetValues_MPIBSG(C, 1, &xrow, 1 , &kstart, val,INSERT_VALUES);	
			}
		}
	}
...
/*** MatGetSubMatrix_MPIBSG : REturns sub matrix
 * Should be strides
...
#undef __FUNCT__  
#define __FUNCT__ "MatGetSubMatrix_MPIBSG"
PetscErrorCode MatGetSubMatrix_MPIBSG(Mat A,IS isrow,IS iscol,MatReuse scall,Mat *B)
{
/*	Mat_MPIBSG    *a = (Mat_MPIBSG*)A->data;
	IS             is1,is2;
	PetscErrorCode ierr;
...
/*	ierr = PetscMalloc2(a->nz,PetscInt,&vary,a->nz,PetscInt,&iary);CHKERRQ(ierr);
	...
	ierr = MatGetSubMatrix_MPIBSG_Private(A,is1,is2,scall,B);CHKERRQ(ierr);
	ierr = ISDestroy(&is1);CHKERRQ(ierr);
	ierr = ISDestroy(&is2);CHKERRQ(ierr);
*/	PetscFunctionReturn(0);
}

/*** MatGetSubMatrices_MPIBSG : REturns sub matrices
 * Should be strides
 * Stride should be same value
 * Range of row and cols should be same
 * */
...
#undef __FUNCT__  
#define __FUNCT__ "MatGetSubMatrices_MPIBSG"
PetscErrorCode MatGetSubMatrices_MPIBSG(Mat A,PetscInt n,const IS irow[],const IS icol[],MatReuse scall,Mat *B[])
{
/*	PetscErrorCode ierr;
	PetscInt       i;
	...
	for (i=0; i<n; i++) {
		ierr = MatGetSubMatrix_MPIBSG(A,irow[i],icol[i],scall,&(*B)[i]);CHKERRQ(ierr);
	}
*/	PetscFunctionReturn(0);
}


/** MatZeroEntries_MPIBSG : Sets the values in the matrix to be zero
Added by Deepan */
#undef __FUNCT__
#define __FUNCT__ "MatZeroEntries_MPIBSG"

PetscErrorCode MatZeroEntries_MPIBSG(Mat A)
{
	PetscErrorCode ierr;
	PetscInt rank, size;
	PetscInt localnz;
	...
	localnz = (a->stencil_local_rend - a->stencil_local_rbeg);
	PetscFunctionBegin;
	memset(a->a,0,sizeof(PetscScalar)*localnz*a->bs*a->stpoints);
	memset(a->diag, 0,sizeof(PetscScalar)*localnz*a->bs);
...
/** MatDiagonalScale_MPIBSG : Scales matrix based on left and right vectors
 * Added by Deepan
 * */
#undef __FUNCT__
#define __FUNCT__ "MatDiagonalScale_MPIBSG"

PetscErrorCode MatDiagonalScale_MPIBSG (Mat A, Vec ll, Vec rr){
/*	Mat_MPIBSG *a = (Mat_MPIBSG *) A->data;
...
#undef __FUNCT__
#define __FUNCT__ "MatView_MPIBSG"
PetscErrorCode MatView_MPIBSG(Mat A,PetscViewer viewer)
{
	PetscErrorCode ierr;
	Mat_MPIBSG * a = (Mat_MPIBSG *)A->data;
	...
	for(count = 0; count < a->nregion; count ++)
	{
		for(icount = a->rstart[count]; icount < a->rstart[count+1] ; icount++)
		{
			...
			for(stcount = 0 ; stcount < a->stpoints; stcount++){
				...
				ierr = PetscViewerASCIISynchronizedPrintf(viewer,"\t %D,( ",stcount);CHKERRQ(ierr);
				for(jcount = 0; jcount < bs ; jcount++)
				{
					ierr = PetscViewerASCIISynchronizedPrintf(viewer,"%G  ",a->coeff[count*a->stpoints + stcount][(icount-a->rstart[count])*bs+jcount]);CHKERRQ(ierr);
				}
				ierr = PetscViewerASCIISynchronizedPrintf(viewer,") \n");CHKERRQ(ierr);
			}
			ierr = PetscViewerASCIISynchronizedPrintf(viewer," ]\n");CHKERRQ(ierr);
