...
#undef __FUNCT__
#define __FUNCT__ "MatCreateFactorType"
/*@C
   MatCreateFactorType - gets the type of factorization it is
...
.seealso:    MatFactorType, MatCreateFactor()
@*/
PetscErrorCode  MatCreateFactorType(Mat mat,MatFactorType *t)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(mat,MAT_CLASSID,1);
  PetscValidType(mat,1);
...
.seealso: MatLUFactorSymbolic(), MatLUFactorNumeric(), MatCholeskyFactor(),
          MatGetOrdering(), MatSetUnfactored(), MatFactorInfo, MatCreateFactor()
...
/*@C
   ...
   Input Parameters:
+  fact - the factor matrix obtained with MatCreateFactor()
.  mat - the matrix
...
.seealso: MatCopy(), MatDuplicate(), MatGetFactorAvailable(), MatCreateFactor()
@*/
...
#undef __FUNCT__
#define __FUNCT__ "MatCreateFactor"
/*@C
   MatCreateFactor - Returns a matrix suitable to calls to MatXXFactorSymbolic()
...
@*/
PetscErrorCode  MatCreateFactor(Mat mat, const MatSolverPackage type,MatFactorType ftype,Mat *f)
{
  ...
  ierr = PetscStrcpy(convname,"MatCreateFactor_");CHKERRQ(ierr);
...
.seealso: MatCopy(), MatDuplicate(), MatCreateFactor()
@*/
...
.seealso: MatGetRow(), MatCreateSubMatrices(), MatGetSubmatrix(), MatGetRowMaxAbs()
@*/
...
.seealso: MatGetDiagonal(), MatCreateSubMatrices(), MatGetSubmatrix(), MatGetRowMaxAbs(),
          MatGetRowMax()
@*/
...
.seealso: MatGetDiagonal(), MatCreateSubMatrices(), MatGetSubmatrix(), MatGetRowMax(), MatGetRowMaxAbs(), MatGetRowMin()
@*/
...
.seealso: MatGetDiagonal(), MatCreateSubMatrices(), MatGetSubmatrix(), MatGetRowMaxAbs(), MatGetRowMin()
@*/
...
.seealso: MatGetDiagonal(), MatCreateSubMatrices(), MatGetSubmatrix(), MatGetRowMax(), MatGetRowMin()
@*/
...
PetscErrorCode MatViewFromOptions(Mat mat,const char prefix[],const char optionname[])
{
  ...
  incall = PETSC_TRUE;
  if (prefix) {
    ierr   = PetscOptionsCreateViewer(PetscObjectComm((PetscObject)mat),prefix,optionname,&viewer,&format,&flg);CHKERRQ(ierr);
  } else {
    ierr   = PetscOptionsCreateViewer(PetscObjectComm((PetscObject)mat),((PetscObject)mat)->prefix,optionname,&viewer,&format,&flg);CHKERRQ(ierr);
  }
  if (flg) {
...
#undef __FUNCT__
#define __FUNCT__ "MatCreateSubMatrices"
/*@C
...
#undef __FUNCT__
#define __FUNCT__ "MatCreateSubMatricesParallel"
PetscErrorCode  MatCreateSubMatricesParallel(Mat mat,PetscInt n,const IS irow[],const IS icol[],MatReuse scall,Mat *submat[])
{
  PetscErrorCode ierr;
  PetscInt       i;
  PetscBool      eq;
...
/*@C
   MatDestroyMatrices - Destroys a set of matrices obtained with MatCreateSubMatrices().
  ...
  if (mat->ops->getsubmatrices && !mat->ops->getsubmatrix && size == 1 && cll == MAT_REUSE_MATRIX) {
    ...
    *newmat = *local;
    ...
    case MAT_INITIAL_MATRIX:
      ierr = MatCreateSubMatrixDefault(mat,isrow,iscoltmp,newmat);CHKERRQ(ierr);
      break;
...
#undef __FUNCT__
#define __FUNCT__ "MatCreateMultiProcBlock"
/*@C
   MatCreateMultiProcBlock - Create multiple [bjacobi] 'parallel submatrices' from
...
.seealso: MatCreateSubMatrices()
@*/
PetscErrorCode   MatCreateMultiProcBlock(Mat mat, MPI_Comm subComm, MatReuse scall,Mat *subMat)
{
  PetscErrorCode ierr;
