...
PetscErrorCode PetscDrawSave_X(PetscDraw draw)
{
  ...
  if (!asv) {
    asv = create_asvisual(drawx->disp, drawx->screen, drawx->depth, 0);if (!asv) SETERRQ(PetscObjectComm((PetscObject)draw),PETSC_ERR_PLIB,"Cannot create AfterImage ASVisual");
  }
